import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

import 'app_colors.dart';

class MyTextStyles {
  // ? Black & DarkBlue fonts
  static TextStyle font24BlackW900 = TextStyle(
      fontSize: 24.sp, fontWeight: FontWeight.w900, color: Colors.black);

  static TextStyle font24DarkBlueW500 = TextStyle(
      fontSize: 24.sp, fontWeight: FontWeight.w500, color: AppColors.darkBlue);

  static TextStyle font14DarkBlueW900 = TextStyle(
      fontSize: 14.sp, fontWeight: FontWeight.w900, color: AppColors.darkBlue);

  static TextStyle font12BlackW500 = TextStyle(
      fontSize: 14.sp, fontWeight: FontWeight.w500, color: Colors.black);

static TextStyle font12BlackW300 = TextStyle(
      fontSize: 14.sp, fontWeight: FontWeight.w300, color: Colors.black);



  static TextStyle font14BlackBlueW400 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.darkBlue,
  );
  static TextStyle font18DarkBlueW700 = TextStyle(
    fontSize: 18.sp,
    fontWeight: FontWeight.w700,
    color: AppColors.darkBlue,
  );

  // ? White fonts
  static TextStyle font18WhiteW500 = TextStyle(
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
    color: Colors.white,
  );

  static TextStyle font14LightGrayW400 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.lightGray,
  );

  static TextStyle font16WhiteW600 = TextStyle(
    fontSize: 16.sp,
    fontWeight: FontWeight.w600,
    color: Colors.white,
  );
  static TextStyle font18WhiteW700 = TextStyle(
    fontSize: 18.sp,
    fontWeight: FontWeight.w700,
    color: Colors.white,
  );

  // ? blue fonts
  static TextStyle font18BlueW500 = TextStyle(
      fontSize: 18.sp, fontWeight: FontWeight.w500, color: AppColors.mainBlue);
  static TextStyle font32BlueW700 = TextStyle(
      fontSize: 32.sp, fontWeight: FontWeight.w700, color: AppColors.mainBlue);
  static TextStyle font12BlueW400 = TextStyle(
      fontSize: 12.sp, fontWeight: FontWeight.w400, color: AppColors.mainBlue);

  static TextStyle font14BlueW400 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.mainBlue,
  );

// ? Gray fonts

  static TextStyle font13GrayW400 = TextStyle(
    fontSize: 13.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.gray,
  );

  static TextStyle font20GrayW500 = TextStyle(
      fontSize: 20.sp, fontWeight: FontWeight.w500, color: AppColors.gray);

  static TextStyle font12GrayW400 = TextStyle(
      fontSize: 12.sp, fontWeight: FontWeight.w400, color: AppColors.gray);
}
